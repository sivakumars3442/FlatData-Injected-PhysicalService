...
namespace FileManager_FlatData.Data
{
    public class AmazonS3FileProvider 
    {
        ...
        public FileManagerResponse<FileManagerDirectoryContent> Delete(string path, string[] names, params FileManagerDirectoryContent[] data)
        {
            RegisterAmazonS3("syncfusion-filemanager", "AKIAWH6GYCX36OQETWF4", "o/kJzgMAHyOuXBVjRpIEYx3jBkBxuxLrHGjJRfK5", "us-east-1");
            return AsyncDelete(path, names, data).Result;
        }

        // Delete aync method
        public virtual async Task<FileManagerResponse<FileManagerDirectoryContent>> AsyncDelete(string path, string[] names, params FileManagerDirectoryContent[] data)
        {
            FileManagerResponse<FileManagerDirectoryContent> removeResponse = new FileManagerResponse<FileManagerDirectoryContent>();
            try
            {
                List<FileManagerDirectoryContent> files = new List<FileManagerDirectoryContent>();
                GetBucketList();
        ...
        public FileManagerResponse<FileManagerDirectoryContent> Copy(string path, string targetPath, string[] names, string[] replacedItemNames, FileManagerDirectoryContent TargetData, params FileManagerDirectoryContent[] data)
        {
        ...
        public FileManagerResponse<FileManagerDirectoryContent> Move(string path, string targetPath, string[] names, string[] replacedItemNames, FileManagerDirectoryContent TargetData, params FileManagerDirectoryContent[] data)
        {
        ...
        public FileManagerResponse<FileManagerDirectoryContent> TransferItems(string path, string targetPath, string[] names, string[] replacedItemNames, bool isCutRequest, FileManagerDirectoryContent TargetData, params FileManagerDirectoryContent[] data)
        {
        ...
        public FileManagerResponse<FileManagerDirectoryContent> Details(string path, string[] names, params FileManagerDirectoryContent[] data)
        {
            ...
            try
            {
                ...
                detailFiles = new FileDetails
                {
                    ...
                    Modified = data.Length == 1 && data[0].IsFile ? data[0].DateModified.ToString() : DateTime.Now.ToString(),
                    Created = data.Length == 1 && data[0].IsFile ? data[0].DateCreated.ToString() : DateTime.Now.ToString(),
        ...
        // Creates a NewFolder
        public FileManagerResponse<FileManagerDirectoryContent> Create(string path, string name, params FileManagerDirectoryContent[] data)
        {
        ...
        public FileManagerResponse<FileManagerDirectoryContent> Search(string path, string searchString, bool showHiddenItems, bool caseSensitive, params FileManagerDirectoryContent[] data)
        {
        ...
        public FileManagerResponse<FileManagerDirectoryContent> Rename(string path, string name, string newName, bool replace = false, bool showFileExtension = true, params FileManagerDirectoryContent[] data)
        {
            return AsyncRename(path, name, newName, replace, showFileExtension, data).GetAwaiter().GetResult();
        }
        public virtual async Task<FileManagerResponse<FileManagerDirectoryContent>> AsyncRename(string path, string name, string newName, bool replace, bool showFileExtension, params FileManagerDirectoryContent[] data)
        {
            GetBucketList();
        ...
        public FileManagerResponse<FileManagerDirectoryContent> Upload(string path, IList<IFormFile> uploadFiles, string action, FileManagerDirectoryContent[] data)
        {
            return AsyncUpload(path, uploadFiles, action, data).Result;
        }

        // Uploads the file(s)
        public virtual async Task<FileManagerResponse<FileManagerDirectoryContent>> AsyncUpload(string path, IList<IFormFile> uploadFiles, string action, FileManagerDirectoryContent[] data)
        {
        ...
        // Returns the image
        public FileStreamResult GetImage(string path, string id, bool allowCompress, params FileManagerDirectoryContent[] data)
        {
            try
            {
        ...
        public virtual async Task<FileStreamResult> DownloadAsync(string path, string[] names, params FileManagerDirectoryContent[] data)
        {
            ...
            if (names.Length == 1 && response.CommonPrefixes.Count == 0)
            {
                try
                {
                    ...
                    Stream stream = fileTransferUtility.OpenStreamAsync(bucketName, RootName.Replace("/", "") + path + names[0]).GetAwaiter().GetResult();
                ...
                try
                {
                    ...
                    using (var archive = new ZipArchive(memoryStream, ZipArchiveMode.Create, true))
                    {
                        foreach (string folderName in names)
                        {
                            ...
                            var initialResponse = GetRecursiveResponse("/", RootName.Replace("/", "") + path + folderName, false).GetAwaiter().GetResult();
                            DownloadSubdirectories(archive, folderName, path + folderName, RootName.Replace("/", "") + path + folderName, initialResponse).GetAwaiter();
                        }
                    }
        ...
        private async Task DownloadSubdirectories(ZipArchive archive, string folderName, string folderPath, string s3FolderPath, ListObjectsResponse response)
        {
            foreach (var item in response.S3Objects)
            {
                ...
                Stream fileStream = fileTransferUtility.OpenStreamAsync(bucketName, s3FilePath).GetAwaiter().GetResult();
                var entry = archive.CreateEntry(filePath, CompressionLevel.Optimal);

                using (var entryStream = entry.Open())
                {
                    fileStream.CopyToAsync(entryStream).GetAwaiter().GetResult();
                }
            }
            foreach (var subdirectory in response.CommonPrefixes)
            {
                string subdirectoryName = subdirectory.Replace(s3FolderPath, "");
                var subdirectoryResponse = GetRecursiveResponse("/", s3FolderPath + subdirectoryName, false).GetAwaiter().GetResult();
                DownloadSubdirectories(archive, folderName, folderName + subdirectoryName, s3FolderPath + subdirectoryName, subdirectoryResponse).GetAwaiter();
            }
        }
        ...
        public async Task MoveDirectoryAsync(string sourceKey, string destinationKey, bool isFile, bool deleteS3Objects)
        {
            try
            {
                ...
                do
                {
                    ListObjectsResponse listObjectsResponse = client.ListObjectsAsync(listObjectsRequest).GetAwaiter().GetResult();
                    foreach (S3Object s3Object in listObjectsResponse.S3Objects)
                    {
                        ...
                        CopyObjectResponse copyObectResponse = client.CopyObjectAsync(copyObjectRequest).GetAwaiter().GetResult();
                ...
                //client.DeleteObjectsAsync(deleteObjectsRequest).GetAwaiter().GetResult();
            }
            catch (AmazonS3Exception amazonS3Exception) { throw amazonS3Exception; }
        }
        ...
        public static async Task ListingObjectsAsync(string delimiter, string prefix, bool childCheck)
        {
            try
            {
                ListObjectsRequest request = new ListObjectsRequest { BucketName = bucketName, Delimiter = delimiter, Prefix = prefix };
                if (childCheck)
                    childResponse = client.ListObjectsAsync(request).GetAwaiter().GetResult();
                else
                    response = client.ListObjectsAsync(request).GetAwaiter().GetResult();
            }
            catch (AmazonS3Exception amazonS3Exception) { throw amazonS3Exception; }
        }
        public string ToCamelCase(FileManagerResponse<FileManagerDirectoryContent> userData)
        {
